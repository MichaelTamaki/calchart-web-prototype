...
describe('Show', () => {
  ...
  beforeAll(() => {
    show = new Show('T Swizzle Show', 10, ShowLabels[0])
  })

  test('initializes correctly', () => {
    expect(show.title).toBe('T Swizzle Show')
    expect(show.stuntSheets).toHaveLength(1)
    expect(show.stuntSheets[0] instanceof Stuntsheet).toBe(true)
    expect(show.stuntSheets[0].dots).toHaveLength(10)
    expect(show.dotLabels).toBe(ShowLabels[0])
  })

  test('create new stuntsheet', () => {
    show.addNewStuntsheet(16)
    expect(show.stuntSheets).toHaveLength(2)
    expect(show.stuntSheets[0].dots).toHaveLength(10)
